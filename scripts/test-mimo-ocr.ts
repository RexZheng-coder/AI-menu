...
if (!imagePath) {
  console.error("Usage: npm run test:mimo:ocr -- ./sample-menu.jpg");
  process.exit(1);
}
...
const { extractMenuTextWithMiMo } = await import("../src/server/mimoOcrExtractor.js");
const image = await createServerMenuImage(imagePath);
const startedAt = Date.now();

try {
  const text = await extractMenuTextWithMiMo([image], {
    timeoutMs: 30_000,
  });
  ...
  console.log("ocr_text_first_1000_chars:");
